...
describe("QuerySupport Tests", function() {
	...
	it("closest is a child element", function(done){		
		let expected = find("#closest-closests-test-1 > span > span > span").first();
		let element = find("#closest-closests-test-1").first().closest("[data-closest-target]").first();		
	...
	it("closest is a parent", function(done){		
		...
		let elements = find("#closest-closests-test-2 > span > span > span").first().closest("[data-closest-target]");		
	...
	it("closest is the element your self", function(done){		
		...
		let element = find("#closest-closests-test-1 > span > span > span").first().closest("[data-closest-target]").first();		
		expect(element).toBe(expected);	
